...
public class TStringParseLongTest extends TStringTestBase {

    @Parameter public TruffleString.ParseLongNode node;

    @Parameters(name = "{0}")
    public static Iterable<TruffleString.ParseLongNode> data() {
        return Arrays.asList(TruffleString.ParseLongNode.create(), TruffleString.ParseLongNode.getUncached());
    }
        ...
        for (long n : new long[]{Long.MIN_VALUE, Long.MIN_VALUE + 1, ((long) Integer.MIN_VALUE) - 1, Integer.MIN_VALUE, Integer.MIN_VALUE + 1, Short.MIN_VALUE, -12345, -1, 0,
            ...
            for (int i = 0; i < radixes.length; i++) {
                ...
                checkAsciiString(strings[i], (a, array, codeRange, isValid, encoding, codepoints, byteIndices) -> {
                    Assert.assertEquals(n, node.execute(a, radix));
                    assertCodePointsEqual(a, encoding, codepoints);
                });
            }
        }
    }
    ...
    public void testNull() throws Exception {
        checkNullS((s) -> node.execute(s, 10));
    }
}
